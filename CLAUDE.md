...
- **Админка `/admin/`** защищена: гард `require_admin` на роутере → любой `/admin/*` без сессии отдаёт **404**. Вход через секретный префикс `/admin/login/<ADMIN_URL_SECRET>` + логин/пароль (`AdminUser`, bcrypt). Креды/секрет — в `.env` (`ADMIN_URL_SECRET`/`ADMIN_USERNAME`/`ADMIN_PASSWORD`), учётка засевается на старте. Логика — `app/services/admin_auth.py`, роуты входа — `auth_router` в `routers/admin/__init__.py`.
